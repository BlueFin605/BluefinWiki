...
| 1.3 layout store + dividers | 3.1 (split position), 3.3 (mobile), 4.1 (inspector width/visible) |
| 1.4 editor keymap | — (self-contained) |
| 1.5 responsive layer (own spec) | mobile variants in 3.4, 3.5, 3.10, 4.1, 6.7 |
...
| 1.5 | [Responsive layer — design pass](step-1.5-responsive-layer.md) | 🔴 (F4) | **Needs its own brainstorm/spec before execution.** |

Order: 1.1 → 1.2 → 1.3 → 1.4 in sequence (1.1/1.2 touch many service files;
avoid merge churn). 1.5 is a planning deliverable, not code yet.

## Phase exit criteria

- [ ] Every step's acceptance criteria met (1.5: sub-spec written + approved).
