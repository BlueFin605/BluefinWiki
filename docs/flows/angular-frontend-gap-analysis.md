...
| **Properties — Title**: click-to-edit inline; typing updates metadata live; **if line 1 is `# H1`, editing the title rewrites that H1**; empty title on blur resets | ✅ | plain always-visible `matInput`; debounced (200 ms) metadata update ✅; **no title↔H1 sync**; **no empty-reset**; no click-to-edit affordance | 🟠 | Port the H1 sync + empty-reset. |
| Properties — **Page Type** select; hidden entirely if no types exist; changing type merges the schema | shown conditionally | always shown ("(none)" + types); `custom-properties-editor` renders the schema fields but merge is display-only and **not persisted unless edited** | 🟠 | Hide when no types; merge schema defaults on save. |
| Properties — **Tags**: chips; `Enter`/`,` add (**lower-cased**, deduped); `Backspace` on empty removes last; **vocabulary autocomplete (`page-tags`, top 5)** | ✅ | `mat-chip-grid`; `Enter`/`,` add; dedupe ✅. **No lower-casing, no Backspace-removes-last, no vocabulary autocomplete.** | 🟠 | Add lower-case, backspace behaviour, and the tag suggestions. |
| Properties — Author / timestamps (read-only, **localized**) | ✅ | shown as **raw** strings (`{{ metadata().createdAt }}`), and `createdBy`/`modifiedBy` are ids | ⚪ | Format dates; resolve ids to names if available. |
| Properties — **Custom Properties** (`CustomPropertiesEditor`): **collapsible**; schema merged with saved values; per-type inputs text/number/date/**tags with vocab autocomplete**; **add ad-hoc property** (kebab name, choose type) + **remove** | ✅ | renders the fixed type-schema fields only; text/number/date/**plain comma-text for tags**; **not collapsible**; **cannot add or remove ad-hoc properties** | 🔴 | Port ad-hoc add/remove, collapsibility, tag vocab. |
...
| **Attachments** (`attachment-manager.ts`) — `GET …/attachments`; **exponential-backoff auto-retry** on load failure (1s→30s, ≤10) + manual "Refresh"; sorted newest-first; per item type emoji, size + date, **image thumbnail** (presigned) → **full-screen lightbox**; actions **Download / Copy Markdown / Drag Link / Insert / Delete**; delete perm = Admin or author or uploader (+`window.confirm`) | ✅ | list name/size/date; **no auto-retry, no Refresh button, no sorting, no thumbnails, no lightbox**; actions = **Insert + Delete only** (no Download, Copy Markdown, Drag Link); delete perm ✅ matches; `window.confirm` ✅ | 🔴 | Port retry + Refresh, sorting, thumbnails/lightbox, and the missing actions. |
| **Attachment upload** (`attachment-uploader.ts`) — client `validateFile` → `presign` → direct S3 `PUT` (progress 10–90) → `confirm`; **sequential**; on success the returned markdown is **auto-inserted at the cursor**; list refreshes; first error inline | ✅ core flow: validate → presign → S3 PUT w/ progress → confirm, sequential ✅ | **markdown is not auto-inserted** — `inspector-panel` only stores `$event.filename`; the user must then click "Insert" on the row. List refreshes via version bump ✅. | 🟠 | Route the uploader's `uploaded` event into `insertMarkdown` at the cursor. |
...
| Inspector (3 tabs, title↔H1 sync, schema/property merge, tag vocab autocomplete, attachment delete perms, attachment backoff retry, backlinks badge) | ⚠️ 3 tabs ✅, delete perms ✅. ❌ title↔H1 sync, ❌ ad-hoc properties, ❌ tag vocab, ❌ attachment backoff retry, ❌ backlinks badge shown, ❌ collapsible custom props. |
| Attachment upload (presign → S3 PUT progress → confirm; sequential; markdown auto-inserted) | ✅ pipeline. ❌ auto-insert of markdown. Also ❌ Download / Copy Markdown / Drag Link / thumbnails / lightbox in the manager. |
